...
import { Shield, Lock, FileCheck, Award, Star } from 'lucide-react';

const TrustSignals: React.FC = () => {
  return (
    <section className="px-6 max-w-7xl mx-auto w-full py-20 border-t border-slate-200">
      <div className="text-center mb-16">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-100 border border-emerald-200 text-emerald-700 text-xs font-semibold mb-6 tracking-wide uppercase">
          <Award size={12} />
          Verified Excellence
        </div>
        <h2 className="text-3xl md:text-5xl font-bold mb-4 text-slate-900">Trusted by the Global Economy</h2>
        <p className="text-slate-600 max-w-2xl mx-auto">
          From solo freelancers to enterprise platforms, thousands rely on Glix to move their money safely and instantly.
        </p>
      </div>
...
const CertBadge: React.FC<{ icon: React.ReactNode; text: string }> = ({ icon, text }) => (
  <div className="flex flex-col items-center gap-3 group cursor-default">
    <div className="p-4 rounded-full bg-slate-50 text-slate-400 border border-slate-200 group-hover:text-emerald-600 group-hover:bg-emerald-50 group-hover:border-emerald-200 transition-all duration-300">
      {React.cloneElement(icon as React.ReactElement<any>, { size: 24 })}
    </div>
    <span className="text-xs font-semibold text-slate-500 group-hover:text-slate-800 transition-colors">{text}</span>
  </div>
);
