    ...
    <section id="ecosystem" className="px-6 max-w-7xl mx-auto w-full py-24 relative">
      <div className="text-center mb-16">
        ...
        <p className="text-slate-600 max-w-2xl mx-auto text-lg">
          Move capital instantly between your Stock Portfolio, Crypto Wallet, and Checking Account. The only financial OS you will ever need.
        </p>
      </div>

      {/* Tabs */}
      <div className="flex justify-center mb-12">
        <div className="flex p-1 bg-white/80 backdrop-blur-md border border-slate-200 rounded-full overflow-x-auto max-w-full shadow-sm">
          {tabs.map((tab) => (
            <button
              key={tab.id}
      ...
      >
        <div className="text-center mb-8">
          <h3 className="text-2xl md:text-3xl font-bold text-slate-900 mb-2">
            We serve over <span className="text-blue-600">170 countries</span> and territories around the world
          </h3>
...
const BentoCard: React.FC<{
  ...
  return (
    <div className="group relative bg-white border border-slate-200 rounded-3xl p-8 hover:shadow-xl transition-all duration-300 hover:scale-[1.02] cursor-pointer overflow-hidden h-full flex flex-col">
      {/* Background Gradient on Hover */}
      <div className={`absolute inset-0 bg-gradient-to-br from-${color}-50 to-${color}-50 opacity-0 group-hover:opacity-100 transition-opacity duration-500`}></div>
      <div className={`absolute top-0 right-0 p-32 bg-${color}-100 blur-[80px] rounded-full pointer-events-none group-hover:bg-${color}-200/50 transition-colors opacity-0 group-hover:opacity-100`}></div>

      <div className="relative z-10 flex flex-col h-full">
        <div className="flex justify-between items-start mb-6">
          <div className={`p-4 rounded-2xl bg-${color}-50 border border-${color}-100 shadow-sm group-hover:scale-110 transition-transform duration-300`}>
            {icon}
          </div>

          {/* Animated Learn More Button */}
          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 scale-75 group-hover:scale-100 transition-all duration-300 translate-x-4 group-hover:translate-x-0 origin-right">
        ...
        <div className="mb-2">
          <span className={`text-xs font-bold uppercase tracking-wider text-${color}-600 mb-2 block`}>{title}</span>
        ...
        {/* Decorative Line */}
        <div className={`w-12 h-1 bg-${color}-500/30 rounded-full mt-6 group-hover:w-full transition-all duration-500`}></div>
      </div>
    </div>
  );
};
